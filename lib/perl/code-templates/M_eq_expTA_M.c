...
static void
QLA_expTA_3x3(QLA3(ColorMatrix,(*e)), QLA3(ColorMatrix,(*iq)))
{
  // flops: 386
  cmplx f0, f1, f2;
  QLA_Complex a0, a1, a2;
...
#undef mul
  ...
  QLA_c_eq_r_plus_ir(a0, creal(f0), cimag(f0));
  QLA_c_eq_r_plus_ir(a1, cimag(f1), -creal(f1));
  QLA_c_eq_r_plus_ir(a2, -creal(f2), -cimag(f2));
...
void
QLAPC(M_eq_expTA_M)(NCARG QLAN(ColorMatrix,(*restrict r)), QLAN(ColorMatrix,(*restrict a)))
{
#ifdef HAVE_XLC
#pragma disjoint(*r, *a)
  __alignx(16,r);
  ...
  if(NC==1) {
    // flops: 0
    QLA_c_eq_r(QLA_elem_M(*r,0,0), 1);
    return;
  }
  if(NC==2) {
    // flops: 17  div: 1  sqrt: 1  cexpi: 1
  ...
  if(NC==3) {
    // flops: 519  div: 2  rdivc: 1  sqrt: 1  acos: 1  cexpi: 3
    QLA_expTA_3x3((QLA3(ColorMatrix,(*))) r, (QLA3(ColorMatrix,(*))) a);
    return;
  }
