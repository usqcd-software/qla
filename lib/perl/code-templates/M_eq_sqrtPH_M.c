      ...
      QLA_Real tr, QLA_Real p2, QLA_Real det)
{
  ...
  // solve x^3 -tr x^2 +0.5(tr^2-p2) x -det = 0
  ...
  if(sq3>fabsP(r)) {
    ...
    //QLA_Real d = u*v-w;
    QLA_Real d = (sl0+sl1)*(sl0+sl2)*(sl1+sl2);
    QLA_Real di = 1./d;
    *f0 = u*w*di;
    *f1 = (tr+v)*di;
    *f2 = -di;
...
#pragma disjoint(*r, *a)
  ...
  if(NC==2) {
    ...
    if(det==0) {  // one zero eigenvalue
      if(tr==0) {  // matrix is zero
	QLA_c_eq_r(QLA_elem_M(*r,0,0), 0);
	QLA_c_eq_r(QLA_elem_M(*r,0,1), 0);
	QLA_c_eq_r(QLA_elem_M(*r,1,0), 0);
	QLA_c_eq_r(QLA_elem_M(*r,1,1), 0);
      } else {
	QLA_Real c1 = 1/sqrtP(fabsP(tr));
	QLA_c_eq_c_times_r(QLA_elem_M(*r,0,0), a00, c1);
	QLA_c_eq_c_times_r(QLA_elem_M(*r,0,1), a01, c1);
	QLA_c_eq_c_times_r(QLA_elem_M(*r,1,0), a10, c1);
	QLA_c_eq_c_times_r(QLA_elem_M(*r,1,1), a11, c1);
      }
    } else {
      QLA_Real sdet, c0, c1;
